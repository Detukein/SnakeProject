...
Zm = (CordeCapteur./2).*sind(gamma)-flechem1.*cosd(gamma);
Ym = -a.*sind(angle2)+flechem2.*cosd(angle2);

%Point de la courbe (2)
% fleche21 = (angle1.*CordeCapteur)./(24.*CordeCapteur.^3).*(31.*CordeCapteur.^3-CordeCapteur.^2/4);
fleche21 = 11/96.*angle1;
% fleche22 = 3.*angle2./8-(2.*angle2)./(3.*CordeCapteur.^3).*(3/4.*CordeCapteur).^3;

X2 = (CordeCapteur./4).*cosd(gamma)-fleche21.*sind(gamma);
Z2 = (CordeCapteur./4).*sind(gamma)-fleche21.*cosd(gamma);
% Y4 = -a./2.*sind(angle2)+fleche22.*cosd(angle2);

%Point de la courbe (4)
% fleche41 = (angle1.*CordeCapteur)./(8.*CordeCapteur.^3).*(3.*CordeCapteur.^3-9/4.*CordeCapteur.^2);
fleche41 = 29/32.*angle1;
% fleche42 = angle2./8-(0.25*CordeCapteur).^2./CordeCapteur.^3.*2/3.*angle2;

X4 = (CordeCapteur./4).*cosd(gamma)-fleche41.*sind(gamma);
Z4 = (CordeCapteur./4).*sind(gamma)-fleche41.*cosd(gamma);
% Z2 = -a./2.*sind(angle2)+fleche42.*cosd(angle2);
...
%      plot3([0, X(i)],[0, Y(i)],[0, Z(i)],'-o');
    plot([0, X2(i), Xm(i), X4(i), X(i)],[0, Z2(i), Zm(i), Z4(i), Z(i)],'-o')
    xlim([-100,100]);
    ylim([-100,100]);
%     zlim([-100,100]);
    drawnow
